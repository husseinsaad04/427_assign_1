...
# Create the database and tables if they don't exist, and add a default user if there are no users.
...
# Add a default user if there are no users in the database
...
# Set up the server socket and bind it to the specified port and wait for a connection. 
...
while True:
    ...
    client, addr = server.accept() #Here we block activity until a client connects
    print("Connected to ", addr) 

    buffer = "" # Buffer to hold incoming data until we get a full command

    while True:

        try: # Wrap the working code in a try block to handle connection errors nicely
            data = client.recv(BUF)# Receive data from the client. This will block until data is received or the connection is closed.
            if not data:
                break
            ...
            while "\n" in buffer:
                ...
                if cmd == "balance":
                    ...
                    if not user:
                        ...
                        client.send(f"\n200 OK: Balance for user {user[0]} {user[1]}: ${user[2]:.2f}\n".encode())

                elif cmd == "list":
                    cur.execute("SELECT ID,stock_symbol,stock_balance,user_id FROM Stocks WHERE user_id=1 ORDER BY ID")
                    rows = cur.fetchall()
                    msg = "\n200 OK: The list of records in the Stocks database for user 1:\n"
                    if len(rows) == 0:
                        msg += "(no stocks)\n"
                    else:
                        for r in rows:
                    ...
                    conn.commit()
                    client.send(f"\n200 OK: BOUGHT: New balance: {new_stock} {symbol}. USD balance ${new_usd:.2f}\n".encode())
                    ...
                    conn.commit()
                    client.send(f"\n200 OK: SOLD: New balance: {new_stock} {symbol}. USD ${new_usd:.2f}\n".encode())
